...
func Downloader(url string) string {
	...
	tempName := fmt.Sprintf("%v/videos/temp_video.mp4", workDir)
	filename := fmt.Sprintf("%v/videos/video%v.mp4", workDir, len(dir))
	err := downloadVideo(url, tempName)
	if err != nil {
		log.Fatalf("Gagal mengunduh video: %v", err)
	}
	log.Println(">>> Video berhasil diunduh :", filename)
	fmt.Println(">>> Start Edit Video : ", filename)
	editvideo.EditVideo(tempName, filename)
	return filename
}
